...
|------|------|--------|---------|-------|--------|---------|---------|-------|-------|
| 1 | ROIC + momentum blend v2: sequential screen — same ROIC/`ret_60d` factor pair, but replace the blended z-score/nanmean combination with a two-step filter (top-half ROIC screen, then rank surviving names by `ret_60d` momentum) instead of averaging the two z-scores | Fundamental × technical combination | Direct follow-up to ROIC + momentum blend (Improve, REVIEW 2026-07-22) — targets the hypothesis that the blend methodology dilutes the ROIC leg's confirmed (both-crisis-window) defensiveness by letting high-momentum/weak-ROIC names clear the top-quartile cutoff on the combined score | 3 | 3 | 3 | 3 | 4 | Both `roic_features` and `technical_features.ret_60d` already live-probed and available (see `strategies/roic_momentum_blend.py`). Screen threshold (top-half vs. top-third ROIC) is an open design choice with real researcher degrees of freedom, hence overfit-risk parallel to the original trial's. Goal is to preserve or strengthen the regime protection (COVID +1.82pts, 2022 bear +4.51pts vs SPY) while improving the decisive IR-gate failure (-0.2205) — not to chase a different mechanism. |
